...
interface CompanyReportsSubStepProps {
  ...
  isActive?: boolean
  siteId?: string | null
  /** Site-scoped saved findings from parent (matches other substeps' pattern).
   *  When the user views "All Sites" this is project-wide; otherwise it's
   *  filtered to the selected site so the same chunk can be saved per-site. */
...
function ResultCard({
  ...
  return (
    <Card
      className={
        saved ? 'border-green-200 bg-green-50/50 dark:border-green-800 dark:bg-green-950/30' : ''
      }
    >
      <CardContent className="p-3">
        ...
        <div className="mb-2 flex items-start justify-between gap-2">
          ...
          <Button
            ...
            className={
              saved
                ? 'shrink-0 hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-950/40 dark:hover:text-red-300'
                : 'shrink-0'
            }
            title={saved ? 'Click to remove' : 'Save to project'}
          >
            {saving ? (
        ...
        {/* Footer: file path */}
        <p className="text-muted-foreground/80 mt-2 text-xs">{result.file_path}</p>
      </CardContent>
    </Card>
  )
}
...
export function CompanyReportsSubStep({
  ...
  // Find an existing saved finding for a given chunk in the current site scope.
  // savedFindings is already filtered by siteId in the parent, so a hit here
  // means the chunk is saved for the currently selected site (or for the whole
  // project when "All Sites" is active).
  const findSavedByChunk = (chunkId: string): DeskResearchFinding | undefined => {
    return savedFindings.find((f) => {
      const raw = f.raw_data as Record<string, unknown> | null
      return raw?.chunkId === chunkId
    })
  }

  const handleToggleSave = async (result: DocumentSearchResult) => {
    setSavingIds((prev) => new Set(prev).add(result.chunk_id))
    try {
      const existing = findSavedByChunk(result.chunk_id)
      if (existing) {
        await deleteFinding.mutateAsync(existing.id)
        toast({ title: 'Removed', description: `Removed finding from ${result.file_name}` })
      } else {
      ...
      toast({ title: 'Error', description: 'Failed to update finding', variant: 'destructive' })
    } finally {
      setSavingIds((prev) => {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {debouncedQuery.length >= 3 && (aiLoading || aiAnswer) && (
          <Card className="mb-4 border-purple-200 bg-purple-50/50 dark:border-purple-800 dark:bg-purple-950/30">
            <CardContent className="p-3">
          ...
          <div className="space-y-3">
            ...
            {results.map((result) => (
              <ResultCard
                ...
                result={result}
                searchQuery={debouncedQuery}
                saved={findSavedByChunk(result.chunk_id) !== undefined}

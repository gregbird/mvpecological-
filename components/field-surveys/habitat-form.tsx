...
export function HabitatForm({
  ...
  const groupedHabitats = React.useMemo(() => {
    ...
    return level1Habitats.map((l1) => ({
      category: l1,
      habitats: FOSSITT_HABITATS.filter(
        (h) => h.level === 3 && h.code.startsWith(l1.code)
      ),
    }));
  }, []);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {initialData?.id ? "Edit Habitat" : "Record Habitat Polygon"}
          </DialogTitle>
          <DialogDescription>
            {initialData?.id
                                ...
                                {group.habitats.map((habitat) => (
                                  ...
                                  >
                                    <div className="flex items-center gap-2 w-full">
                                      ...
                                      />
                                      <span className="font-mono text-sm w-10 shrink-0">
                                        {habitat.code}
                                      </span>

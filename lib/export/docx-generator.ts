      ...
      if (a === 'designated_sites' && ad && ad.designatedSites.length > 0) {
        children.push(
          buildDocxAppendixTable(
            ...
            ad.designatedSites.map((s) => [s.name, `${s.siteNumber} (${s.siteType})`, s.distanceKm])
          )
        )
        ...
        children.push(
          new Paragraph({
            children: [
              new TextRun({
                text: 'This appendix is reserved for manual content. Insert the relevant map, photographs, datasheets or reference material after export.',
                italics: true,
                color: '888888',
              }),
            ],

 ...
 */
export function wgs84ToItm(lat: number, lng: number): { easting: number; northing: number } {
  ...
  const Ma = (1 + n + (5 / 4) * n2 + (5 / 4) * n3) * (latRad - lat0)
  const Mb = (3 * n + 3 * n2 + (21 / 8) * n3) * Math.sin(latRad - lat0) * Math.cos(latRad + lat0)
  const Mc =
    ((15 / 8) * n2 + (15 / 8) * n3) * Math.sin(2 * (latRad - lat0)) * Math.cos(2 * (latRad + lat0))
  const Md = (35 / 24) * n3 * Math.sin(3 * (latRad - lat0)) * Math.cos(3 * (latRad + lat0))
 ...
 */
export function itmToWgs84(easting: number, northing: number): { lat: number; lng: number } {
  const { a, b, F0, lat0, lng0, E0, N0 } = ITM
  ...
  const XIIA =
    (secLat / (5040 * Math.pow(nu, 7))) * (61 + 662 * tan2lat + 1320 * tan4lat + 720 * tan6lat)

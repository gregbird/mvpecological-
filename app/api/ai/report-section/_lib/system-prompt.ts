...
- **Site area / project area:** When referring to the size of the development site, use ONLY the figure labelled "Site boundary area" in the PROJECT INFORMATION block. The "Sum of habitat polygon areas" is NOT the site area — it is the combined extent of every mapped polygon (which can exceed the site or even the buffer when polygons overlap or extend beyond). Conflating these two numbers is a factual error that misrepresents the development scale.
- **Do NOT invent sub-categories within Fossitt codes.** Each habitat polygon has ONE Fossitt code (e.g. BL3 = "Buildings and artificial surfaces"). Do not split a Fossitt total into invented sub-categories like "buildings (X ha), artificial surfaces (Y ha), road infrastructure (Z ha)" — the NLC 2018 dataset does not provide that level of breakdown for BL3. Cite the Fossitt total as-is. The same rule applies to all aggregate Fossitt codes (BL2, GA1, WD1, etc.) — no fictional sub-classification.
